...
import { SectionCard, Countdown, StatTile } from "@/components/eleven/SectionCard";
...
export default function Draft() {
  ...
  const banner = useMemo(() => {
    ...
    return {
      ...
      detail:
        summary.currentDeadline !== null
          ? "El pool se actualiza al instante: si alguien ficha a un jugador, desaparece de tu lista."
          : "Espera tu turno: el pool se actualiza al instante con cada adquisición.",
    };
  }, [summary, myTurn]);
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-5">
      ...
      {/* Banner: the state machine speaking in one sentence. */}
      <div
        role="status"
        className={cn(
      ...
      >
        <banner.icon
          ...
          className={cn(
            ...
            banner.tone === "positive" && "text-emerald-600 dark:text-emerald-300",
            ...
            <p className="mt-1 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
              ...
              {summary.currentDeadline !== null ? (
                <span className="num normal-case tracking-normal">
                  {" "}
                  · cierra en {Math.max(0, Math.ceil((summary.currentDeadline - now) / 1000))}s
                </span>
              ) : null}
            </p>
          ) : null}
      ...
      {summary ? (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          ...
          />
          <StatTile icon={Search} label="Pool libre" value={`${summary.poolSize}`} />
          <StatTile
            icon={ShoppingCart}
      ...
      {draftActive ? (
        <SectionCard
          title={myTurn ? "Jugadores disponibles · elige tu fichaje" : "Jugadores disponibles"}
          icon={Gavel}
          action={
            <span className="text-xs text-muted-foreground">
              {pool === undefined ? "Cargando…" : `${pool.length} mostrados`}
            </span>
          }
        >
          <div className="mb-4 grid gap-2 sm:grid-cols-3">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select
                value={group}
                onValueChange={(value) => setGroup(value as PositionGroup | "todos")}
                disabled={!myTurn}
              >
          ...
          {!myTurn ? (
            <p className="mt-4 flex items-start gap-2 rounded-lg border border-border bg-muted/40 p-2.5 text-xs text-muted-foreground">
              ...
              puedes explorar el estado del draft y el historial de adquisiciones.
            </p>
          ) : null}
        </SectionCard>
      ) : null}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-[11px] text-muted-foreground">
                      ...
                      {relativeTime(pickRow.pickedAt, now)}
...
/** Confirmation with the same checks the server will run. */
function PickDialog({
  player,
  open,
  onOpenChange,
